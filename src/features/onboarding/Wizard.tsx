...
import { Sparkles, MapPin, School, ShieldCheck, User } from 'lucide-react';
  ...
  return (
    <div className="w-full max-w-5xl mx-auto px-4 py-6 sm:py-10">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-7 bg-[#0e1320]/80 border border-white/[0.08] backdrop-blur-2xl rounded-3xl p-6 sm:p-8 shadow-2xl relative overflow-hidden">
        ...
        {/* Right / Live Student Pass Preview (Desktop only) */}
        <aside aria-label="Live Profile Preview" className="hidden lg:block lg:col-span-5 sticky top-8 space-y-4">
          <div className="flex items-center justify-between px-2">
            <span className="text-[11px] font-bold uppercase tracking-wider text-slate-400 flex items-center gap-1.5">
              <Sparkles className="w-3.5 h-3.5 text-[#ff385c]" /> Live Campus ID Preview
            </span>
            <span className="text-[10px] font-mono text-[#ff385c] bg-[#ff385c]/10 border border-[#ff385c]/25 px-2 py-0.5 rounded-full font-semibold">
              Step {state.currentStep}/4
            </span>
          </div>

          {/* Luxury Holographic Identity Card */}
          <div className="glass-card rounded-3xl p-6 border border-white/[0.1] shadow-2xl relative overflow-hidden">
            {/* Ambient inner glow */}
            <div className="absolute top-0 right-0 w-40 h-40 bg-gradient-to-br from-[#ff385c]/20 to-transparent rounded-full blur-2xl pointer-events-none" />
            ...
            <div className="flex items-center justify-between border-b border-white/[0.08] pb-4 mb-4">
              <div className="flex items-center space-x-2.5">
                ...
                <div>
                  <span className="text-xs font-bold text-white tracking-tight block">Nuvora Pass</span>
                  <span className="text-[10px] text-slate-400 block font-mono">Verified Student</span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-1 text-[10px] text-emerald-400 font-semibold bg-emerald-500/10 border border-emerald-500/25 px-2 py-0.5 rounded-full">
                ...
                <span>{state.draft.emailVerified ? 'Email Verified' : 'In Progress'}</span>
              </div>
            </div>

            {/* Avatar & Name */}
            <div className="flex items-start space-x-4 mb-5">
              <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-slate-800 to-slate-900 border border-white/[0.12] flex items-center justify-center text-slate-300 shadow-inner font-bold text-lg shrink-0">
              ...
              <div className="space-y-1 min-w-0">
                <h4 className="text-base font-bold text-white tracking-tight truncate">
                  {state.draft.fullName || 'Your Full Name'}
                </h4>
                <p className="text-xs text-[#ff758c] font-medium font-mono">
                  {state.draft.pronouns || 'pronouns'} {state.draft.age ? `• Age ${state.draft.age}` : ''}
                </p>
            ...
            {/* Campus Info */}
            <div className="space-y-2.5 p-3.5 rounded-2xl bg-[#090d16]/70 border border-white/[0.05] text-xs">
              <div className="flex items-center gap-2 text-slate-300">
                <MapPin className="w-3.5 h-3.5 text-[#ff385c] shrink-0" />
                <span className="truncate font-medium">
                  {state.draft.city ? `${state.draft.city}, ${state.draft.state}` : 'Location (City, State)'}
                </span>
              </div>
              ...
              <div className="flex items-center gap-2 text-slate-300">
                ...
                <span className="truncate">
                  {state.draft.college || 'Affiliated College / Campus'}
                </span>
              </div>
            </div>
            ...
            <div className="mt-4 pt-3 border-t border-white/[0.06]">
              <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider block mb-2">
                Selected Vibes ({selectedVibeItems.length}/5)
              </span>
